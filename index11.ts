...
interface Person {
  ...
  let person: Person = { name: 'John', age: 30 }; // Error: Type '{ name: string; age: number; }' is not assignable to type 'Person'.
  
  // Define a structurally compatible object
  interface Employee {
    name: string;
    age: number;
  }
  ...
  let person2: Person = employee; // Error: Type 'Employee' is not assignable to type 'Person'.
  
  // Define a nominally compatible object
  interface AnotherPerson {
    name: string;
    age: number;
  } 
  
  let anotherPerson: AnotherPerson = { name: 'Bob', age: 30 };
  
  // Assign the AnotherPerson object to a Person variable
  let person3: Person = anotherPerson; // Okay
