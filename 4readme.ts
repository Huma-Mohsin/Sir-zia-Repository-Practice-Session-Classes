...
function getFirstThree(x: number[] | string[]| string) {
  return x.slice(0, 3);
}
...
let x4: string | number ="America";

if (typeof x4 === 'string') {
  const y = x4 as string;
...
let x6: { foo?: string } = {};// x6 is an object with an optional property foo of type string.
...
// In this example, the type of x9 is narrowed to string within the if block because the condition x9 is truthy. If x9 were falsy (e.g., null or undefined), the type of x would not be narrowed.

// Truthiness narrowing is a useful feature in TypeScript that helps you write more precise and safe code by refining types based on the truthiness of values.
